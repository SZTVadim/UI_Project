from test_UI_svs_selenium.utils.test_data import text_email_already_exists, text_required_input_error, text_account_created_message


def test_create_account_valid_case(register_page):  # Указать НЕ ЗАРЕГИСТРИРОВАННЫЙ email
    register_page.first_page_open()
    register_page.submit_registration_form(firstname='Ted', lastname='Testov', email='toov@test.com',
                                           password='Qwerty123.', confirm_password='Qwerty123.')
    register_page.assert_account_created_message(text_account_created_message)
...
def test_existing_email(register_page):
    ...
    register_page.assert_email_already_exists(text_email_already_exists)
...
def test_empty_first_name(register_page):
    ...
    register_page.assert_required_input_error('first_name', text_required_input_error)
