...
def test_create_account_valid_case(customer_account):  # Указать не зарегистрированный email
    customer_account.open_page(url=customer_account.page_url)
...
def test_existing_email(customer_account):
    customer_account.open_page(url=customer_account.page_url)
    customer_account.filling_out_form(firstname='Ted', lastname='Testov', email='tttesttoov@test.com', password='Qwerty123.',
